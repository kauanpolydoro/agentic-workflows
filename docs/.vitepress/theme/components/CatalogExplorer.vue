...
import type { GeneratedCatalogRecipe } from "@agentic-workflows/core";
  ...
  <section class="catalog-explorer" aria-labelledby="catalog-filter-heading">
      ...
      <div class="filter-heading">
        <div>
          ...
          <p id="catalog-filter-help">
            Filters combine with AND. Compatibility defaults to compatible recipes only.
          </p>
        </div>
        <button
          class="reset-button"
    ...
    <div v-if="filteredRecipes.length > 0" class="workflow-grid">
      ...
      >
        ...
        <h2>{{ recipe.title }}</h2>
        <p>{{ recipe.summary }}</p>
        <span class="card-link">Inspect workflow <span aria-hidden="true">→</span></span>
      </a>
    </div>

    <div v-else class="empty-state">
      <h2>No matching workflows</h2>
      <p>Change one or more filters, or reset them to return to the full catalog.</p>
