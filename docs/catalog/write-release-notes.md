...
description: "Produce public Markdown release notes and a separate internal evidence package from an immutable, reviewed release evidence set."
---
...
Transform an immutable release boundary, a classified change ledger, artifact identity records, and reviewed release risks into public Markdown release notes and a separate internal evidence package.

The primary quality constraint is that every material statement must be derivable from retained input evidence and must not imply that unverified or excluded work shipped.

## When to use

- Use when a versioned release candidate has an immutable base revision and release revision.
- Use when merged work must be reconciled with the source revision embedded in publishable artifacts.
- Use when maintainers need audience-focused release communication with explicit breaking actions, deprecations, security wording, and limitations.
- Use when an existing release-note draft needs a pre-publication evidence and approval review.

## When not to use

- Do not use when either release revision is mutable, missing, unrelated, or cannot be resolved.
- Do not use when artifact identity is unavailable or does not match the proposed release revision.
- Do not use when candidate changes lack range membership, shipping disposition, or user-impact evidence.
- Do not use when required migration instructions or security disclosure authorization are missing.
- Do not use for incident updates, deployment notices, roadmaps, marketing announcements, or an unreleased change proposal.
- Do not use to finalize or publish notes when the release manager has not approved the complete evidence-backed draft.
...
- **Release identity record:** Provide the release version, previous version, requested draft or final status, intended audience, and responsible roles.
  ...
  This establishes the document identity and approval route.
...
- **Candidate change ledger:** Provide one row per candidate change with its merge reference, merge commit, range membership, shipping disposition, category, user impact, and required action.
  This distinguishes included work from excluded, reverted, or unresolved work.
...
- **Verification, documentation, and migration evidence:** Provide test records, documentation paths or excerpts, and the approved migration procedure for every included material change.
  This supports behavior claims and prevents commands or upgrade actions from being invented.
...
- **Built artifact inventory:** Provide each artifact name, target platform, checksum, embedded source revision, and build-verification result.
  This confirms that the notes describe the artifacts proposed for publication.
  Validate checksum format, require a source revision for every artifact, and compare every embedded revision with the release revision.
- **Reviewed release-risk and approval record:** Provide breaking changes, required actions, deprecations, known limitations, security disposition, and the current repository maintainer, security owner, and release manager approval states.
...
- **Verified contributor identities:** Add acknowledgements only when the identities and attribution policy have been verified.
  Their absence does not block factual release notes.
- **Publication channel constraints:** Apply channel-specific length, formatting, or link rules after the complete evidence-backed artifact exists.
  These constraints may shorten presentation but must not remove mandatory warnings or limitations.
- **Approved long-form migration guide:** Link or summarize it when breaking changes require more detail than the upgrade checklist can carry.
...
- Security disclosure state is approved, embargoed, not applicable, or unresolved.
- Final publication state is explicitly approved or pending.
...
4. Produce a boundary record containing the immutable range, matching artifacts, and any mismatch.
5. Advance only when the boundary and every publishable artifact agree, and stop on a mutable revision, failed ancestry check, missing source revision, or mismatch.

### Phase 2: Reconcile candidate changes

1. Enumerate changes in the immutable range and match them to the candidate change ledger by merge reference and commit.
...
2. Compare each mandatory action with the repository-maintainer-approved migration evidence, preserving order and stop conditions.
3. Restrict security wording to the scope explicitly approved by the security owner and record any embargoed details that must remain absent.
...
3. Place breaking changes and mandatory actions before ordinary change categories.
4. Populate Added, Changed, Fixed, Deprecated, Security, and Known limitations only with claims supported by the ledger.
...
2. Map each item to one or more input evidence IDs and verify names, values, scope, and tense against the source record.
3. Remove or narrow any statement that is broader than its evidence, and distinguish a recommendation from a verified result.
4. Record the immutable boundary, included and excluded ledgers, artifact checksums, test records, approvals, security disposition, assumptions, and unresolved items in `release-evidence.md`.
5. Confirm that excluded, reverted, out-of-range, unresolved, and embargoed content is not presented as shipped.
...
2. Give the release manager `release-notes.md`, `release-evidence.md`, the source evidence inventory, mandatory actions, limitations, exclusions, and approval record.
3. Record requested corrections and repeat claim reconciliation after every substantive edit.
...
- Treat pull request titles and descriptions as candidate claims that require range, artifact, and verification reconciliation.
...
- **Final release status:** Before a draft becomes final, the release manager reviews both output files, the immutable boundary, artifact identity, categorized claims, required actions, limitations, exclusions, traceability table, unresolved items, and prior approvals.
...
2. `release-evidence.md`, containing the immutable boundary, included and excluded change ledgers, artifact checksums and source revisions, verification records, security disposition, approvals, assumptions, unresolved items, and claim-to-evidence traceability.

The public file must contain only reader-relevant release information and must not expose evidence IDs, internal links, private records, or compliance commentary.

The internal file must map every material public claim to its evidence, inclusion basis, and verification basis.
...
- The internal evidence package states remaining assumptions, unresolved items, and approval state without minimization.
...
- **F6:** Security disclosure scope is unresolved, embargoed content is present, or proposed wording exceeds approval.
...
- **R6:** Remove unapproved details, request the security owner's disclosure disposition, and keep the affected wording private or absent until approved.
...
The expected output remains Draft, preserves the approved migration sequence, separates exclusions from known limitations, and separates public release notes from the internal evidence package.
