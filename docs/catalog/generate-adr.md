...
description: "Produce a reviewable ADR that separates facts from assumptions, compares viable options consistently, records accountable approval, and defines consequences and revisit triggers."
---
...
Transform a stable architecture question, attributable evidence, and accountable approval state into a complete decision record with comparable alternatives, explicit consequences, owned follow-up, and measurable revisit triggers.
The ADR must distinguish observed facts, assumptions, preferences, recommendations, approvals, and unexecuted implementation work.

## When to use

- Use this workflow when a choice has durable consequences for system boundaries, data ownership, integration contracts, operational dependencies, security posture, or several consumers.
- Use it when at least two viable alternatives can be evaluated against the same decision drivers.
- Use it to propose a decision, retain an approved decision, reject a proposal, or explicitly supersede a related ADR.

## When not to use

- Do not create an ADR when the problem boundary, mandatory constraints, affected systems, or accountable owner role remains unknown.
- Do not use an ADR for a local, low-cost, easily reversible implementation choice that does not constrain future work.
- Do not use this workflow to rationalize a preselected option while withholding a viable alternative or material disadvantage.
- Do not mark status Accepted, Rejected, or Superseded when retained owner evidence is absent.
- Do not use an ADR as a substitute for an incident record, implementation plan, API specification, or security exception process.
...
- **Stable decision boundary, affected systems, current state, and explicit exclusions:** state exactly one decision, what changes if it is made, which systems and consumers are affected, and which adjacent questions remain out of scope.
  This prevents one ADR from hiding several unrelated choices.
...
- **Attributable facts, constraints, assumptions, open questions, and decision drivers:** identify the source and integrity of each fact, distinguish mandatory constraints from preferences, label every assumption, and list unresolved questions.
  ...
  Do not turn an unsupported preference into a constraint.
- **Viable alternatives evaluated against common criteria with concrete disadvantages:** provide at least two genuinely possible options, including no change when it is credible, and apply the same defined criteria to each.
  ...
  Validate that no option is intentionally underspecified to manufacture a preferred result.
- **Accountable decision owner, approval state, related decisions, and supersession relationship:** provide the approving role, the exact current status, retained review or approval evidence, and links or identifiers for related ADRs.
  Status must remain Proposed when approval is absent.
  Supersession requires an explicit old-to-new relationship and migration consequence.

## Optional inputs

- Reproducible benchmarks, incident records, cost models, or security reviews can strengthen a driver when their methods, scope, and provenance are retained.
  Otherwise label the statement as an assumption or omit it.
- Reversible experiment results can reduce uncertainty without converting a pilot into production proof.
- Implementation sequencing and migration constraints can make consequences actionable, but they do not prove that implementation occurred.
...
1. Write a one-sentence decision question, affected systems, current state, desired outcome, and exclusions.
   Assign a provisional ADR identifier only through the repository's naming convention.
   Advance when the owner role confirms that this is one durable decision; stop and split a compound question.
2. Build an evidence ledger that separates facts, mandatory constraints, preferences, assumptions, and open questions.
   ...
   Advance only when each material context claim can be sourced or explicitly labeled as an assumption.
...
4. Define common criteria before scoring or selecting an option.
   Mark each criterion as mandatory, preferred, or informational and define what satisfactory evidence looks like.
   Stop if the criteria encode an option name or cannot be applied consistently.
5. Describe every viable option at comparable depth, including ownership, dependencies, operational model, migration effort, reversibility, and material disadvantages.
...
6. Evaluate every option against every criterion using evidence, stated inference, or known uncertainty.
   Produce a comparison table and sensitivity note for assumptions that could change the result.
   If no option satisfies all mandatory criteria, defer selection and define evidence or scope work.
...
7. State the recommended or selected option and explain each reason through the decision drivers.
   Keep status Proposed until retained owner approval exists.
...
8. Record immediate and long-term positive, negative, and neutral consequences.
   Include new ownership, coupling, operational burden, migration work, failure modes, and explicitly accepted tradeoffs.
...
9. Define follow-up actions with owner roles, dependencies, exit criteria, and verification signals.
   Define rollback or supersession behavior for implementation when applicable.
   Separate decision completion from implementation completion.
10. Define concrete revisit triggers, such as a failed mandatory criterion, changed constraint, new consumer class, cost threshold, incident pattern, or unavailable dependency becoming supported.
...
11. Submit the boundary, evidence ledger, common-criteria table, consequences, unknowns, follow-up actions, and revisit triggers to the architecture owner.
    Record the actual decision and status without inventing a date or consensus.
...
12. Validate links, identifiers, status, related ADR relationships, sensitive content, and evidence traceability.
    Publish the ADR only when completion criteria are reviewable and the repository's documentation gate is satisfied.

## Decision points

- If the question contains multiple independently reversible choices, split it into related ADRs before evaluating options.
- If owner approval is absent, retain status Proposed regardless of author recommendation or apparent consensus.
...
- If rationale contains restricted security or commercial detail, publish a safe summary and retain the sensitive evidence in its approved access-controlled location.
...
- Before superseding an existing decision, the architecture owner reviews the old and new boundaries, replacement rationale, compatibility or migration consequences, implementation state, and links in both records.
...
- identifier or repository-relative filename when supplied, precise title, and actual status;
- decision boundary, affected systems, current state, exclusions, and related decisions;
...
- recommendation or approved decision with evidence-backed rationale;
...
- approval record and any explicitly non-applicable gates;
- measurable revisit and supersession triggers;
- limitations and evidence traceability.

The ADR is complete as a decision artifact even when implementation has not started, provided its status and follow-up state say so explicitly.
...
- All known material consequences and uncertainties have a disposition, owner role, or revisit trigger.
...
- The ADR does not claim implementation, measurement, consensus, or approval beyond supplied evidence.
...
- **R2:** Restate or split the boundary, rebuild common criteria, and reevaluate every option before retaining any recommendation.
...
- **R4:** Reopen the comparison, add the consequence or related decision, reassess the selected option, and seek renewed owner disposition.
...
The [synthetic input](#complete-example-input) supplies a stable validation-ownership question, repository facts, common criteria, alternatives, related-decision search, and explicit architecture-owner approval.
