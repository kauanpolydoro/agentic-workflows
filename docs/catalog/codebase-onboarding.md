...
description: "Produce a role-specific guide from an immutable repository snapshot with verified setup, traced architecture, safe edit boundaries, and a bounded first contribution."
---
...
The [synthetic input](#complete-example-input) supplies a fictional TypeScript ledger repository, exact clean-clone results, observed request flow, ownership records, generated boundaries, and an approved starter task.
The [complete expected output](#complete-expected-output) demonstrates a publishable guide while keeping Docker-dependent integration validation explicitly unexecuted.
