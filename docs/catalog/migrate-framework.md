...
description: "Move a versioned application to a supported framework through coexistence, contract checkpoints, measured traffic stages, and retained rollback."
---
...
The [synthetic input](#complete-example-input) supplies an Express-to-Fastify service inventory, completed health pilot, staging observation, rollback drill, approval states, and blocked later integrations.
The [complete expected output](#complete-expected-output) demonstrates a finished pilot checkpoint dossier without claiming production traffic or whole-migration completion.
